 ...
 * @author catalina
 */
public class SquareEnix extends Thread{
    private Semaphore mainMutex = new Semaphore(1);

    private int dayCounter;
    // JUEGO ESTANDAR 
    private int juegoSGuion = 1;
    private int juegoSNiveles = 1;
    private int juegoSSprites = 2;
    private int juegoSSistemasComplejos = 4;
    private int gananciaS = 350;

    // JUEGO CON DLC: 
    private int maxParaDlc = 2; // lo que se necesita para crear dlc 
    private int cantDlc = 3; // la cantidad de dlc que se tienen que crear
    private int gananciDlc = 700;
...
//    private String companyName= "Square Enix";
    ...
    public SquareEnix() {
        ...
        this.DesarroladorNarrativa = new Worker(this.getDrive(), 4, 10, 1, this.getNarrativa(), this.getDayDuration(), "narrative", this.getMainMutex(),"Square Enix");
        this.DiseñadorNiveles = new Worker(this.getDrive(), 4, 13, 1, this.getNiveles(), this.getDayDuration(), "levels", this.getMainMutex(), "Square Enix");
        this.ArtistaSprites = new Worker(this.getDrive(), 1, 20, 1, this.getSprites(), this.getDayDuration(), "sprites", this.getMainMutex(), "Square Enix");
        this.ProgramadorLogica = new Worker(this.getDrive(), 1, 8, 5, this.getLogica(), this.getDayDuration(), "systems", this.getMainMutex(), "Square Enix");
        this.DesarrolladorDlc = new Worker(this.getDrive(), 2, 17, 1, this.getDlc(), this.getDayDuration(), "dlc", this.getMainMutex(), "Square Enix");
        this.miIntegrador = new Integrador(2, this.getDayDuration(), 1, 1, 2, 4, 3, this.getDrive(), this.getMainMutex(), this.getIntegrador());
        this.director = new Director(this.getDayCounter(), this.getDayDuration(), this.getMiIntegrador(), 700, 340, this.getMainMutex(), "Square Enix");
        this.pm = new PM(this.getDayDuration(), this.getDirector(), this.getMainMutex(), "Square Enix");
    ...
    public void leerTxt() {
        ...
        String path = "src//Txt//SquareEnix.txt";
...
//                         System.out.println("");
//                        System.out.println(lectura[1]);
                        setNiveles(java.lang.Integer.parseInt(lectura[1]));
                    } else if ("sprites".equals(lectura[0])) {
//                         System.out.println("");
//                        System.out.println(lectura[1]);
                        setSprites(java.lang.Integer.parseInt(lectura[1]));
                    } else if (lectura[0].equals("logica")) {
//                         System.out.println("");
//                        System.out.println(lectura[1]);
                        setLogica(java.lang.Integer.parseInt(lectura[1]));
                    } else if (lectura[0].equals("dlc")) {
//                         System.out.println("");
//                        System.out.println(lectura[1]);
                        setDlc(java.lang.Integer.parseInt(lectura[1]));
                    } else if (lectura[0].equals("dayCounter")) {
                        this.setDayCounter(Integer.parseInt(lectura[1]));
                    } else {
//                         System.out.println("");
//                        System.out.println(lectura[1]);
                        setIntegrador(java.lang.Integer.parseInt(lectura[1]));
                    }
...
//        System.out.println("Integer: " + this.integrador);
    ...
    public void run(){
        ...
        try {this.getDesarroladorNarrativa().start();
        ...
        this.getPm().start();
          ...
          this.costoAcumulado += this.DesarroladorNarrativa.getSalaryAcumulado() + this.DesarrolladorDlc.getSalaryAcumulado() + this.DiseñadorNiveles.getSalaryAcumulado()+this.ArtistaSprites.getSalaryAcumulado() + this.ProgramadorLogica.getSalaryAcumulado() +this.miIntegrador.getSalary() + this.director.getSalary() + this.pm.getSalaryAcumulado();   
            Main.p.salarioMonth2.setText(Integer.toString(this.costoAcumulado));
